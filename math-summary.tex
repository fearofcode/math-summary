...
Let $a \in \mathbb{R}$. Let $S = \{r \in \mathbb{R} : 0 \leq r < a\}$. Then we claim $\sup S = a$. $S$ is not empty as, e.g., $a - 1 \in S$. So by the least upper bound property, $\sup S$ exists. Let $r = \sup S$. We are claiming that $r = a$. We prove this by contradiction. If $r \neq a$, either $r < a$ or $r > a$. For the first case, suppose $r < a$. Then let $s =(a - r)/2$. Then $r < s < a$, so $s \in S$. But then $r$ would not actually be an upper bound, a contradiction. Now suppose $r > a$. But then $a$ is an upper bound for $S$ and $a < r$, contradicting the minimality of $r$. So we must have that $r = a$.
\end{example}

The least upper bound property does not apply to, e.g. $\mathbb{Q}$. Any subset of $\mathbb{R}$ whose least upper bound is $\sqrt{2}$ is an example of this, such as Example \ref{supexample} in the previous paragraph with $a = \sqrt{2}$. A lot of expository material in lower-division mathematics just assumes some field $\mathbb{F}$, and the results apply to any such field, including $\mathbb{Q}$ and $\mathbb{R}$. But any result predicated on the least upper bound theory does not apply to the rationals.

Constructing sets and taking their minimum or maximum element is a common tactic in mathematics. It's great for creating proofs that an average undergraduate student would never think of!
